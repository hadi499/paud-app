<script>
  ...
  // const images = [
  //   "https://api.dicebear.com/7.x/fun-emoji/svg?seed=Budi",
  //   "https://api.dicebear.com/7.x/fun-emoji/svg?seed=Ani",
  //   "https://api.dicebear.com/7.x/fun-emoji/svg?seed=Cita",
  //   "https://api.dicebear.com/7.x/fun-emoji/svg?seed=Dodi",
  //   "https://api.dicebear.com/7.x/fun-emoji/svg?seed=Eko",
  //   "https://api.dicebear.com/7.x/fun-emoji/svg?seed=Fina",
  // ];
  const images = [
    ...
    "/images/doraemon/d5.png",
    "/images/doraemon/d6.png",
  ];
...
<div
  class="max-w-xl mx-auto mt-12 p-6 bg-gray-50 rounded-2xl shadow-lg text-center select-none"
>
  ...
  <div class="min-h-[220px] flex flex-col items-center justify-center">
    {#if gameState === "start"}
      <div class="flex flex-col items-center gap-5 w-full">
        <p class="text-lg font-bold text-gray-700">Sudah siap bermain?</p>

        <!-- Time selector -->
        <div class="bg-blue-100 p-4 rounded-xl w-full">
          <p class="mb-2 font-bold text-blue-700">Pilih Waktu Menghafal:</p>

          <div class="flex justify-center gap-2">
            {#each [10, 7, 5] as time}
              <button
                class={`px-4 py-2 rounded-full font-bold border-2 transition
                ${
                  selectedTime === time
        ...
        <button
          class="bg-orange-500 hover:bg-orange-600 active:translate-y-1 text-white px-8 py-3 text-lg font-bold rounded-full shadow-md"
          onclick={startGame}
        >
          ▶ MULAI MAIN
        </button>
    ...
    {:else}
      <p
        class={`text-lg font-bold mb-2 ${gameState === "result" ? "text-blue-600" : "text-gray-700"}`}
      >
        {message}
      </p>
      ...
      {#if targetCard}
        <div class="my-3">
          <img
            ...
            class="w-28 h-28 mx-auto bg-white border-4 border-dashed border-yellow-400 rounded-xl p-2"
            draggable="false"
            alt=""
          />
        </div>
      ...
      {#each cards as card}
        ...
        >
          ...
          >
            <!-- Front -->
            <div
              class="absolute w-full h-full flex items-center justify-center bg-white border-4 border-gray-200 rounded-xl shadow-md backface-hidden"
            >
              <img
                src={card.imageUrl}
            ...
            <!-- Back -->
            <div
              class="absolute w-full h-full flex items-center justify-center bg-yellow-400 border-4 border-yellow-500 rounded-xl shadow-md text-white text-5xl font-bold rotate-y-180 backface-hidden"
            >
              {card.number}
            </div>
          </div>
